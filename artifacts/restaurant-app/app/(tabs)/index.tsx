...
export default function HomeScreen() {
  ...
  const gridTitle = searchQuery
    ...
    : selectedCategory
    ? categories.find((c) => c.id === selectedCategory)?.name ?? t("tabMenu")
    : t("allMeals");
  ...
  return (
    ...
    >
        ...
        <>
          ...
          >
            <View style={styles.bannerText}>
              ...
              <View style={styles.bannerBtn}>
                <Text style={[styles.bannerBtnText, { color: colors.primary }]}>{t("orderNow")}</Text>
              </View>
            </View>
          ...
          <View style={styles.sectionHeader}>
            ...
            <Pressable onPress={() => router.push("/(tabs)/categories")}>
              <Text style={[styles.seeAll, { color: colors.primary }]}>{t("seeMore")}</Text>
            </Pressable>
          </View>
          ...
          <View style={[styles.sectionHeader, { marginTop: 20 }]}>
            <Text style={[styles.sectionTitle, { color: colors.foreground }]}>{t("popularMeals")}</Text>
      ...
      {/* Items Grid */}
      <View style={[styles.sectionHeader, { marginTop: 20 }]}>
        <Text style={[styles.sectionTitle, { color: colors.foreground }]}>{gridTitle}</Text>
      </View>

      {displayedItems.length === 0 ? (
        <EmptyState icon="search" title={t("noMealsFound")} subtitle={t("tryDifferentSearch")} />
      ) : (
...
const styles = StyleSheet.create({
  ...
  banner: {
    ...
    flexDirection: "row", height: 140, marginBottom: 20,
  },
  ...
  bannerBtn: {
    marginTop: 8, backgroundColor: "#fff", alignSelf: "flex-start",
